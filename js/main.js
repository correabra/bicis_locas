...
function validateForm(){
	/* primero obtengo el valor de los campos del formulario */
	var name = $('#name').val().trim();	
	var mayuscula = new RegExp('^[A-Z]+$', 'i');
	var tipobici = $('#tipo-bici').val().trim();
	...
	var inputpassword = $('#input-password').val().trim();
	if(inputpassword == ''){
		$('#input-password').parent().append('<span>Debe ingresar una contraseña</span>');
	}
	else{
		if(inputpassword =='password'||inputpassword =='123456'||inputpassword =='098754'){
